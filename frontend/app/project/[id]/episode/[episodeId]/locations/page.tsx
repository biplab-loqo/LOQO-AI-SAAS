...
import { MapPin, ChevronLeft, Loader2, ImageIcon, Eye } from 'lucide-react'
import { cn } from '@/lib/utils'
import { apiClient, type LocationOut } from '@/lib/api-client'
...
function parseLocation(row: LocationOut): ParsedLocation {
  const desc = row.location_description || {}
  const name = String(
    desc.display_name || desc.location_name || desc.name_identifier || desc.name || 'Unknown'
  )
...
export default function LocationsPage() {
  ...
  useEffect(() => {
    let alive = true
    setLoading(true)
    apiClient.getEpisodeLocations(episodeId)
      .then(rows => {
        if (!alive) return
        setLocations(rows.map(parseLocation))
      })
      .catch(err => {
        if (alive) setError(err?.message ?? 'Failed to load locations')
      })
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            <div className="w-16 h-16 rounded-2xl bg-emerald-500/10 flex items-center justify-center">
              <MapPin className="w-8 h-8 text-emerald-400/40" />
            </div>
            <p className="text-base font-semibold text-foreground">No Locations Yet</p>
            <p className="text-sm text-muted-foreground">Locations will appear here once generated in a part.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {locations.map((loc, idx) => (
              <motion.div key={loc.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}
                className="rounded-2xl border border-border/40 bg-card overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                {/* Location name header */}
                <div className="px-5 py-4 border-b border-border/20 bg-gradient-to-r from-emerald-500/5 to-transparent">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-emerald-500/15 flex items-center justify-center flex-shrink-0">
                      <MapPin className="w-5 h-5 text-emerald-400" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <h3 className="text-base font-bold text-foreground truncate">{loc.name}</h3>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-secondary text-muted-foreground">v{loc.version}</span>
                        {loc.isApproved && (
                ...
                <div className="px-5 py-3 space-y-1 border-b border-border/10">
                  {Object.entries(loc.description)
                ...
                {/* Anchor images */}
                {loc.anchorImages.length > 0 && (
                  <div className="px-5 py-3 border-b border-border/10">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-400 mb-2">Anchor Images</p>
                    <div className="grid grid-cols-2 gap-2">
                      {loc.anchorImages.map((img, i) => (
                ...
                {/* Reference images */}
                {loc.viewPackImages.length > 0 && (
                  <div className="px-5 py-3">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-sky-400 mb-2">Reference Images</p>
                    <div className="grid grid-cols-3 gap-1.5">
                      {loc.viewPackImages.slice(0, 6).map((img, i) => (
                        <ImageCard key={i} url={img.url} label={img.label} />
                      ))}
                    </div>
                    {loc.viewPackImages.length > 6 && (
                      <p className="text-[10px] text-muted-foreground mt-1.5">+{loc.viewPackImages.length - 6} more</p>
                    )}
                  </div>
                )}
                ...
                {loc.collageImage && (
                  <div className="px-5 py-3 border-t border-border/10">
                    ...
                    <ImageCard url={loc.collageImage.url} label={loc.collageImage.label} />
                  </div>
                )}
              </motion.div>
            ))}
